...
async function tickOnce(session: SessionState): Promise<void> {
  ...
  if (session.lastClosedBarT === bar.t) {
    const forming = n >= 1 ? bundle.m30[n - 1]! : bar;
    const gapMs = forming.t - bar.t;
    if (gapMs > M30_MS * 1.5) {
      const nextClose = new Date(forming.t + M30_MS).toISOString().slice(11, 16);
      console.error(
        `[forward-demo] ${new Date().toISOString().slice(11, 19)} market break / bar gap (${Math.round(gapMs / 60000)}m) — next closed bar ~${nextClose} UTC`
      );
    } else {
      console.error(
        `[forward-demo] ${new Date().toISOString().slice(11, 19)} waiting for new M30 bar (last: ${new Date(bar.t).toISOString().slice(11, 16)})`
      );
    }
    return;
  }
