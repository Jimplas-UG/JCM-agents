...
export async function publishTradeExecuted(opts: {
  ...
  return publishJcmEvent('trade_executed', {
    ...
    raw_payload: {
      setup: opts.setup,
      bar_time_ms: opts.barTimeMs,
      mt5_ticket: opts.mt5Ticket,
    },
  });
}

export async function publishTradeClosed(opts: {
  symbol: string;
  direction: 'long' | 'short';
  lotSize: number;
  entryPrice: number;
  ...
  const pips =
    opts.entryPrice && opts.exitPrice
      ? Math.abs(opts.exitPrice - opts.entryPrice) * (opts.symbol.includes('XAU') ? 10 : 10000)
      : null;
  return publishJcmEvent('trade_closed', {
    event_id: newEventId('bilshenz-close'),
    ...
    raw_payload: {
      mt5_ticket: opts.mt5Ticket,
      open_event_id: opts.openEventId,
      closed_at_ms: opts.closedAtMs ?? Date.now(),
    },
  });
}
