...
export function evaluateValidationAlerts(
  ...
  const rejectRate =
    (live.rejectedOrders / Math.max(1, live.trades + live.rejectedOrders + live.missedTrades)) * 100;
  if (rejectRate > thresholds.maxExecutionMismatchRatePct) {
    alerts.push({
      code: 'ORDER_REJECT_RATE',
      severity: 'CRITICAL',
      message: `Order reject rate ${rejectRate.toFixed(1)}% > ${thresholds.maxExecutionMismatchRatePct}% (MT5/infra — not strategy)`,
      value: rejectRate,
      threshold: thresholds.maxExecutionMismatchRatePct,
    });
  }

  if (live.executionMismatches > 0) {
    const mismatchRate =
      (live.executionMismatches / Math.max(1, live.trades + live.executionMismatches)) * 100;
    if (mismatchRate > thresholds.maxExecutionMismatchRatePct) {
      alerts.push({
        code: 'EXECUTION_MISMATCH',
        severity: 'CRITICAL',
        message: `Fill vs intent mismatch rate ${mismatchRate.toFixed(1)}% > ${thresholds.maxExecutionMismatchRatePct}%`,
        value: mismatchRate,
        threshold: thresholds.maxExecutionMismatchRatePct,
      });
    }
